function slCellStyle(sl, targetPct) {
  if (sl === null) return { backgroundColor: '#f9fafb', color: '#d1d5db' }
  const hue = Math.max(0, Math.min(120, sl * 1.2))
  return {
    backgroundColor: `hsl(${hue}, 60%, 93%)`,
    color:           `hsl(${hue}, 55%, 28%)`,
    fontWeight:      sl >= targetPct ? 700 : 600,
    outline:         sl >= targetPct ? `2px solid hsl(${hue}, 55%, 55%)` : 'none',
    outlineOffset:   '-2px',
  }
}

export default function SensitivityGrid({ data, baseAgents, slTargetPct, loading, label }) {
  const isSprinkleSL = data?.gridType === 'sprinkleSL'
    ...
    <div className="bg-white rounded-2xl shadow-md p-6 space-y-4">
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-xs text-gray-400">
          {isSprinkleSL
            ? 'Projected SL · Erlang-C · outlined cells meet target'
            : 'Est. hourly SL · ~60 runs per column'}
        </span>
      </div>
      ...
      {!loading && data && isSprinkleSL && (
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                <th className="text-left py-2 pr-3 text-xs font-medium text-gray-400 uppercase tracking-wide whitespace-nowrap">
                  Hour
                </th>
                <th className="py-2 px-3 text-xs font-medium text-gray-400 uppercase tracking-wide text-right whitespace-nowrap">
                  Baseline
                </th>
                {data.sprinkleCounts.map(s => (
                  <th key={s} className="py-2 px-3 text-xs font-semibold text-center text-gray-500 border-r border-gray-100 whitespace-nowrap">
                    +{s}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {data.hours.map((hour, h) => (
                <tr key={h} className="border-t border-gray-50">
                  <td className="py-1.5 pr-3 text-gray-500 whitespace-nowrap text-xs">{hour}</td>
                  <td className="py-1.5 px-3 text-right text-xs text-gray-500">
                    {data.baselineAvail[h].toFixed(1)}
                  </td>
                  {data.grid[h].map((sl, i) => (
                    <td
                      key={i}
                      className="py-1.5 px-3 text-center text-xs border-r border-gray-50"
                      style={slCellStyle(sl, slTargetPct)}
                    >
          ...
          <div className="flex items-center gap-2 mt-3">
            <div className="flex rounded overflow-hidden h-3 w-24">
              {Array.from({ length: 12 }, (_, i) => (
                <div key={i} className="flex-1" style={{ backgroundColor: `hsl(${i * 10}, 60%, 93%)` }} />
              ))}
            </div>
            <span className="text-xs text-gray-400">Low → High SL</span>
